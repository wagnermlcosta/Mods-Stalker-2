...
# - Para nodes de NÍVEL SUPERIOR: adicionar ArmorPiercing e CoverPiercing com valor igual 4

INPUT_FILE = "NPCWeaponSettingsPrototypes.cfg"
OUTPUT_FILE = "NPCWeaponSettingsPrototypes_patch_GlassCannon.cfg"

# ========== PRIMEIRA PASSAGEM: EXTRAIR VALORES ORIGINAIS ==========
piercing_values = {}  # Dicionário para armazenar valores originais de ArmorPiercing e CoverPiercing por nome de node

with open(INPUT_FILE, "r", encoding="utf-8") as f:
    lines_first_pass = f.readlines()

current_node_name = None
current_armor_piercing = 1.0  # valor padrão
current_cover_piercing = 1.0  # valor padrão
...
for i, line in enumerate(lines_first_pass):
    ...
    if "struct.begin" in stripped:
        if nesting_level_first == 0:
            ...
            if ':' in line:
                ...
                current_armor_piercing = 1.0  # reset para padrão
                current_cover_piercing = 1.0
    ...
    if stripped.endswith("struct.end"):
        ...
        if nesting_level_first == 0 and current_node_name:
            ...
            piercing_values[current_node_name] = {
                'armor': current_armor_piercing,
                'cover': current_cover_piercing
            }
            current_node_name = None
        continue
    
    if current_node_name and nesting_level_first == 1:
        # Procurar pelos valores de ArmorPiercing e CoverPiercing
        if "ArmorPiercing =" in stripped:
            value_str = stripped.split("=")[1].strip().rstrip('.')
            current_armor_piercing = float(value_str)
        elif "CoverPiercing =" in stripped:
            value_str = stripped.split("=")[1].strip().rstrip('.')
            current_cover_piercing = float(value_str)

# Aplicar regra simétrica: se um existe e o outro não, usar o mesmo valor
for node_name, values in piercing_values.items():
    if values['armor'] != 1.0 and values['cover'] == 1.0:
        # ArmorPiercing existe mas CoverPiercing não
        piercing_values[node_name]['cover'] = values['armor']
    elif values['cover'] != 1.0 and values['armor'] == 1.0:
        # CoverPiercing existe mas ArmorPiercing não
        piercing_values[node_name]['armor'] = values['cover']
...
for line in lines:
    ...
    if "struct.begin" in stripped:
        ...
        if begin_pos != -1:
            ...
            if is_top_level and current_node_name_pass2 and current_node_name_pass2 in piercing_values:
                # Detectar indentação padrão (3 espaços baseado no arquivo original)
                param_indent = "   "
                
                # Definir valores fixos como 4.0
                armor_piercing = 4.0
                cover_piercing = 4.0
...
for sid, values in piercing_values.items():
    print(f"  {sid}: ArmorPiercing=4.0, CoverPiercing=4.0")